...
[@caporaso2011global], which is publicly accessible through phyloseq
[@mcmurdie2013phyloseq].
...
using complete linkage on the UniFrac distance. Figure \@ref(gptimebox) displays
the resulting hierarchy together with a parallel coordinates view of the
$\text{asinh}$ transformed taxa.

```{r gptimebox, fig.cap = "An application to the Global Patterns demonstrates how linking in treelapse can be applied to combine hierarchical clustering and parallel coordinates views."}
...
clustering tree, after zooming to a few of the microbes that distinguish between
...
the distinguishing microbes confirms this. For example, many of the microbes
